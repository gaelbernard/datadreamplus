...
class DataPrep:
    ...
    def run(self, embeddings=None, map=None, translate=False, umap_cloud_path='new_umap_n40_c0.99_i0.pkl', delete=False, device=None):
        '''
    ...
    def _embed(self, delete, batch_size=100, device=None):
        ...
        for i in range(start_index, size, batch_size):
            ...
            pd.DataFrame(model.encode(ldf['text'].values, device=device), index=ldf.index.tolist()).to_pickle(f'{folder}/{i}.pkl')
